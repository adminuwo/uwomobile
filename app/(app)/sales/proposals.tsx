...
export default function ProposalsScreen() {
  ...
  const handleShare = (id: string, num: string) => {
    Share.share({
      message: `View Proposal #${num} from Unified Web Options: https://uwoconnect.aisa24.com/public/proposal/${id}`,
    });
  };
  ...
  return (
    <Screen safeAreaEdges={['top', 'left', 'right']}>
      ...
      {isLoading ? (
        ...
        <View style={styles.center}>
          <Text color={colors.error}>Failed to load proposals.</Text>
        </View>
      ) : filteredProposals.length === 0 ? (
        <View style={styles.emptyState}>
          <FileText size={48} color={colors.textMuted} style={{ marginBottom: 16 }} />
          <Text variant="h3" color={colors.textPrimary}>No Proposals Found</Text>
          <Text variant="body" color={colors.textMuted} style={{ textAlign: 'center', marginTop: 8 }}>
            Click 'New Proposal' above to build, send, and track a client pitch proposal.
          </Text>
        </View>
      ) : (
        <FlatList
